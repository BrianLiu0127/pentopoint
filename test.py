...
import keyboard
import pandas as pd
from edgesdk import EdgeDevice
...
class MapList(QWidget):
        ...
        while (1):
            ...
            if self.ShoppingCartButton:
                ...
                if self.formLayout.rowCount() != len(self.ShoppingCartBuyed):
                    ...
                    btn.setStyleSheet(
                        "background-color:#DDDDDD;font-size:14px;color:#000000;text-decoration:line-through;")
    ...
    def setList(self):
        ...
        for items in ShopingList:
            ...
            target.setStyleSheet(
                "font-size:10px;background-color:rgba(245, 223, 77,1);font-size:12px;border:1px solid #000;border-radius: 15px;")
    ...
    def showitem(self: Item):
        ...
        if (maplist.ShoppingCartLabel[idx].isHidden() or (maplist.show_item == self) or (
                maplist.show_item == None and idx == 0)):
            ...
            if (isnotclass == 0):
                maplist.ShoppingCartLabel[idx].setText(self.text() + "\n可能在" + self.shelf)
                maplist.ShoppingCartPoint[idx].setStyleSheet(
                    "background-color:rgba(0,255,0,0.3);border-radius: 10px;border:1px solid #000")
                if (self.shelf == "飲料"):
                ...
                maplist.ShoppingCartLabel[idx].move(round(x - maplist.ShoppingCartLabel[idx].width() / 2),
                                                    round(y - maplist.ShoppingCartLabel[idx].height() / 2))
